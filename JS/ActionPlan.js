document.addEventListener("DOMContentLoaded", async function () {
 ...
 // ======================================================
 const API_BASE = ""; // Asegúrate de colocar tu URL base aquí si es necesario
 ...
 function getUsernameUsuarioActionPlan() {
   const currentUser = getUsuarioSesionActionPlan();
   if (!currentUser) return "";
   return String(currentUser.username || currentUser.usuario || currentUser.user || currentUser.name || currentUser.nombre || "").trim();
 }
 function esUsuarioAASOLISActionPlan() {
   return getUsernameUsuarioActionPlan().toUpperCase() === "AASOLIS";
 }
...
<option value="">Todas / Selecciona Business Unit</option>
 ...
 function llenarProductionLinesSelect(idBusinessUnit = "", valorSeleccionado = "") {
   ...
   if (idBusinessUnit) lines = lines.filter((pl) => String(pl.idBusinessUnit) === String(idBusinessUnit));
...
<option value="">Todas / Selecciona Production Line</option>
 ...
 function obtenerIdBusinessUnitSeleccionada() {
   ...
   return (option && option.value) ? (option.dataset?.id || "") : "";
 }
 ...
 // ======================================================
 // RESPONSABLES Y PREGUNTAS
 // ======================================================
 async function cargarResponsablesActionPlan() {
   responsablesActionPlan = [];
   try {
     const res = await fetch(`${API_BASE}/api/usuarios?t=${Date.now()}`, { method: "GET", cache: "no-store", headers: { Accept: "application/json" } });
     const data = await res.json().catch(() => ({}));
     ...
     responsablesActionPlan = usuarios.filter((u) => {
       const activo = u.activo ?? u.Activo ?? u.active ?? u.isActive ?? 1;
       return String(activo) === "1" || activo === true || String(activo).toLowerCase() === "true";
     }).map((u) => String(u.nombre ?? u.Nombre ?? u.name ?? u.Name ?? u.usuario ?? u.Usuario ?? u.username ?? "").trim()).filter((nombre) => nombre !== "");
...
<option value="">Selecciona una pregunta</option>
 ...
 function obtenerNombreModuloSeleccionado() {
   ...
   return option && option.value ? (option.dataset?.nombre || option.textContent?.trim() || "") : "";
 }
 function obtenerTextoPreguntaSeleccionada() {
   if (!hallazgoPregunta) return "";
   const option = hallazgoPregunta.options[hallazgoPregunta.selectedIndex];
   return option && option.value ? (option.dataset?.texto || option.textContent?.trim() || "") : "";
 }
 // ======================================================
 // ACTION PLANS - LÓGICA PRINCIPAL
 // ======================================================
 ...
 function filtrarItems(items) {
   ...
   return items.filter((item) => {
     ...
     const textoCompleto = normalizarTexto([item.fecha, item.creadoPor, item.businessUnit, item.productionLine, item.modulo, item.pregunta, item.accionRequerida, item.comentarios, item.responsable, item.fechaCompromiso, item.fechaCierre, estadoCalculado].join(" "));
 ...
 function setStatusCardActive(status) {
   ...
   statusCards.forEach((card) => {
     if (String(card.dataset.status || "").toUpperCase() === statusActual) card.classList.add("active");
 ...
 function renderTable() {
   ...
   tablaBody.innerHTML = items.map((item) => {
     ...
     return `
<tr>
<td class="fecha-cell">${escapeHTML(formatDate(item.fecha))}</td>
<td class="creado-por-cell">${escapeHTML(item.creadoPor || "Usuario")}</td>
<td class="bu-cell">${escapeHTML(item.businessUnit)}</td>
<td class="pl-cell">${escapeHTML(item.productionLine)}</td>
<td class="modulo-cell">${escapeHTML(item.modulo)}</td>
<td class="text-wrap-cell pregunta-cell">${escapeHTML(item.pregunta)}</td>
<td class="text-wrap-cell accion-cell">${escapeHTML(item.accionRequerida)}</td>
<td class="text-wrap-cell comentarios-cell">${escapeHTML(item.comentarios || "")}</td>
<td class="text-wrap-cell responsable-cell">${escapeHTML(item.responsable)}</td>
<td class="fecha-compromiso-cell">${escapeHTML(formatDate(item.fechaCompromiso))}</td>
<td class="fecha-cierre-cell">${escapeHTML(formatDate(item.fechaCierre))}</td>
<td class="estado-cell">
<span class="estado-badge ${getEstadoClass(estadoCalculado)}">${escapeHTML(estadoCalculado)}</span>
</td>
...
<button type="button" class="btn btn-sm btn-outline-primary" onclick="editarHallazgo('${item.id}')" title="Editar"><i class="fa-solid fa-pen"></i></button>
             ${esAdminActionPlan() ? `<button type="button" class="btn btn-sm btn-outline-danger" onclick="eliminarHallazgo('${item.id}')" title="Ocultar"><i class="fa-solid fa-trash"></i></button>` : ""}
</div>
</td>
</tr>
     `;
 ...
 async function guardarHallazgo() {
   ...
   const estadoCalculado = calcularEstadoAutomatico(fechaCompromisoValue, fechaCierreReal, esNuevo ? "ABIERTO" : itemAnterior?.estado || "ABIERTO");
   const nuevo = {
     id: id || "",
   ...
   try {
     ...
     if (!res.ok || json.ok === false) throw new Error(json.detalle || json.error || json.message || text || `HTTP ${res.status}`);
 ...
 // ======================================================
 // EXPORTACIÓN Y ACCIONES GLOBALES (Window)
 // ======================================================
 function exportarActionPlanExcel() {
   ...
   worksheet["!cols"] = [{ wch: 14 }, { wch: 24 }, { wch: 18 }, { wch: 30 }, { wch: 24 }, { wch: 65 }, { wch: 50 }, { wch: 45 }, { wch: 30 }, { wch: 18 }, { wch: 18 }, { wch: 14 }];
 ...
 window.eliminarHallazgo = async function (id) {
   if (!esAdminActionPlan()) return alert("Solo un administrador puede ocultar planes de acción.");
   if (!confirm("¿Seguro que deseas ocultar este plan de acción?\n\nNo se borrará de la base de datos, solamente dejará de mostrarse en la app.")) return;
   try {
     const usuarioActual = getUsuarioActualActionPlan();
     const rolActual = getRolUsuarioActionPlan();
     const res = await fetch(`${API_ACTION_PLANS}/${encodeURIComponent(id)}/ocultar`, {
       method: "PATCH",
       headers: { "Content-Type": "application/json", Accept: "application/json", "x-user-role": rolActual, "x-user": usuarioActual },
       body: JSON.stringify({ usuario: usuarioActual, rol: rolActual })
     });
     const text = await res.text();
     let json = {};
     try { json = JSON.parse(text); } catch { json = { ok: false, error: text }; }
     if (!res.ok || json.ok === false) throw new Error(json.detalle || json.error || json.message || `HTTP ${res.status}`);
     await cargarActionPlansDesdeBD();
   } catch (error) {
     console.error("Error ocultando plan:", error);
     alert("No se pudo ocultar el plan de acción.\n\nDetalle: " + (error.message || "Error desconocido"));
   }
 };
 window.eliminarHallazgoPermanente = async function (id) {
   if (!esUsuarioAASOLISActionPlan()) return alert("Solo AASOLIS puede eliminar permanentemente planes de acción.");
   if (!confirm("ADVERTENCIA:\n\nEsta acción eliminará el plan de acción directamente de la base de datos.\nEsta acción NO se puede deshacer.\n\n¿Deseas continuar?")) return;
   try {
     const usuarioActual = getUsernameUsuarioActionPlan();
     const res = await fetch(`${API_ACTION_PLANS}/${encodeURIComponent(id)}`, {
       method: "DELETE",
       headers: { "Content-Type": "application/json", Accept: "application/json", "x-user": usuarioActual },
     ...
     console.error("Error eliminando permanentemente plan:", error);
     alert("No se pudo eliminar permanentemente el plan de acción.\n\nDetalle: " + (error.message || "Error desconocido"));
   }
 };
 // ======================================================
 // EVENT LISTENERS
